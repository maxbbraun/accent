...
Accent is a smart picture frame with a pop of color and no cables. Read more about it [on Medium](https://accent.ink).
...
3. Obtain a Google Maps Secret Generator secret from [Credentials](https://console.cloud.google.com/google/maps-apis/credentials) by selecting Maps Static API from the dropdown, and add in the `api_keys` collection under `/api_keys/google_maps/url_secret`.
4. Obtain an [OAuth client ID](https://console.developers.google.com/apis/credentials) for the [Google Calendar API](https://developers.google.com/calendar/quickstart/python) with scope `https://www.googleapis.com/auth/calendar.readonly`.
